...
export async function sincronizarPagosMP(usuarioId: string): Promise<number> {
  ...
  for (const pago of pagos) {
    ...
    // - account_fund: siempre INGRESO (fondos entrando a la billetera)
    // - payer_id presente y no es nuestro usuario: alguien nos mandó dinero → INGRESO
    // - collector_id presente y es nuestro usuario: somos el receptor → INGRESO
    // - resto: GASTO (nosotros enviamos/pagamos)
    const esIngreso =
      pago.operation_type === 'account_fund' ||
      (pago.payer?.id != null && String(pago.payer.id) !== conexion.mpUsuarioId) ||
